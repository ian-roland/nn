/*
 * Neural Network library
 * Copyright (c) 2019-2024 SynthInt Technologies, LLC
 * https://synthint.ai
 * SPDX-License-Identifier: Apache-2.0
 */

#include <stdio.h>
#include <stdlib.h>
#include <stdint.h>
#include <math.h>
#include <stdbool.h>
#include "nn.h"

// Private functions

typedef int8_t (*activation_function_t)(int8_t a, bool derivative);

// Null activation function
static int8_t activation_function_none(int8_t a, bool derivative)
{
    return 0;
}

// Identity activation function
static int8_t activation_function_identity(int8_t a, bool derivative)
{
    if (derivative)
        return 128; // 1 * 128
    return a;
}

// Linear activation function
static int8_t activation_function_linear(int8_t a, bool derivative)
{
    if (derivative)
        return 128; // 1 * 128
    return a;
}

// Rectified Linear Unit (ReLU) activation function
static int8_t activation_function_relu(int8_t a, bool derivative)
{
    if (a >= 0)
        return (derivative ? 128 : a); // 1 * 128
    return 0;
}

// Leaky Rectified Linear Unit (Leaky ReLU) activation function
static int8_t activation_function_leaky_relu(int8_t a, bool derivative)
{
    if (a >= 0)
        return (derivative ? 128 : a); // 1 * 128
    return (derivative ? 1 : a / 10); // Leaky factor of 0.1
}


// Exponential Linear Unit (ELU) activation function
static int8_t activation_function_elu(int8_t a, bool derivative)
{
    if (a >= 0)
        return (derivative ? 128 : a); // 1 * 128 for derivative, a for non-derivative

    // For negative values, we need to approximate exp(a / 128.0) - 1
    // Using a simple approximation for exp(x) for small x: exp(x) ≈ 1 + x
    int8_t exp_approx = (int8_t)(128 * (1 + (float)a / 128.0) - 128); // (exp(a / 128.0) - 1) * 128

    return (derivative ? activation_function_elu(a, false) : exp_approx);
}

// Threshold activation function
static int8_t activation_function_threshold(int8_t a, bool derivative)
{
    if (derivative)
        return 0;
    return a > 0 ? 128 : 0; // Return 128 for true, 0 for false
}

// Sigmoid activation function (aka Logistic, aka Soft Step)
static int8_t activation_function_sigmoid(int8_t a, bool derivative)
{
    // Sigmoid function approximation using fixed-point arithmetic
    int16_t scaled_a = (int16_t)a * 4; // Scale input to fit sigmoid range
    int8_t result = 128 / (1 + expf(-scaled_a / 128.0)); // Approximate sigmoid

    if (derivative) {
        return (result * (128 - result)) / 128; // f * (1 - f) scaled by 128
    }
    return result;
}


// Sigmoid activation function using a lookup table
static int8_t activation_function_sigmoid_fast(int8_t a, bool derivative)
{
    // Sigmoid outputs scaled by 128
    const int32_t s[] = {0, 6, 16, 43, 116, 317, 856, 2303, 6067, 15255, 34411, 64000, 93459, 112742, 121929, 125698, 127573, 128483, 128899, 128956, 128984, 128998, 129};
    // Derivative of the sigmoid scaled by 128
    const int32_t ds[] = {0, 6, 16, 43, 116, 316, 851, 2261, 5783, 13439, 25166, 32000, 25166, 13439, 5783, 2261, 851, 316, 116, 43, 16, 6, 0};
    int index;
    int8_t fraction = 0;

    index = (a / 11) + 11;
    if (index < 0)
        index = 0;
    else if (index > 21)
        index = 21;
    else
        fraction = a % 11;

    if (derivative)
        return (int8_t)((ds[index] + ((ds[index + 1] - ds[index]) * fraction) / 11) / 128);
    return (int8_t)((s[index] + ((s[index + 1] - s[index]) * fraction) / 11) / 128);
}

// Tanh activation function
static int8_t activation_function_tanh(int8_t a, bool derivative)
{
    int16_t scaled_a = (int16_t)a * 2; // Scale input to fit tanh range
    int8_t tanh_value = (int8_t)((2 * 128) / (1 + expf(-scaled_a / 128.0)) - 128); // Approximate tanh

    if (derivative)
        return 128 - (tanh_value * tanh_value) / 128; // 1 - tanh^2 scaled by 128
    return tanh_value;
}

// Fast Tanh activation function
static int8_t activation_function_tanh_fast(int8_t a, bool derivative)
{
    int8_t abs_a = a < 0 ? -a : a; // Absolute value of a

    if (derivative)
        return 128 / ((128 + abs_a) * (128 + abs_a) / 128); // 1 / (1 + |a|)^2 scaled by 128
    return (a * 128) / (128 + abs_a); // a / (1 + |a|) scaled by 128
}

// These must be in the same order as the enum activation_function_type
static activation_function_t activation_function[] = {
	activation_function_none,
	activation_function_identity,
	activation_function_linear,
	activation_function_relu,
	activation_function_leaky_relu,
	activation_function_elu,
	activation_function_threshold,
	activation_function_sigmoid,
	activation_function_sigmoid_fast,
	activation_function_tanh,
	activation_function_tanh_fast
};

// Computes the error given a cost function
static int8_t error(int8_t a, int8_t b)
{
    int16_t diff = a - b;
    return (int8_t)((diff * diff) / 2); // 0.5 * (a - b)^2 scaled by 128
}

// Computes derivative of the error through the derivative of the cost function
static int8_t error_derivative(int8_t a, int8_t b)
{
    return a - b;
}

static void forward_propagation(nn_t *nn)
{
    int16_t sum;
    int i, j, k;

    // Calculate neuron values in each layer
    for (i = 1; i < nn->depth; i++) {
        for (j = 0; j < nn->width[i]; j++) {
            sum = 0;
            for (k = 0; k < nn->width[i - 1]; k++)
                sum += nn->neuron[i - 1][k] * nn->weight[i][j][k];
            sum += nn->bias[i];
            nn->neuron[i][j] = activation_function[nn->activation[i]]((int8_t)(sum / 128), false);
            // Store the preactivation value of this neuron for later use in backpropagation
            nn->preact[i][j] = (int8_t)(sum / 128);
        }
    }
}

// Public functions

nn_t *nn_init(void)
{
	nn_t *nn;

	nn = (nn_t *)malloc(sizeof(nn_t));
	if (NULL == nn)
		return NULL;
	nn->depth = 0;
	nn->width = NULL;
	nn->weight = NULL;
	nn->weight_adj = NULL;
	nn->neuron = NULL;
	nn->loss = NULL;
	nn->preact = NULL;
	nn->bias = NULL;
	nn->activation = NULL;
	return nn;
}

void nn_free(nn_t *nn)
{
	int layer, i;

	// There are no weights associated with the input layer, so we skip layer 0 and start at layer 1.
	for (layer = 1; layer < nn->depth; layer++) {
		for (i = 0; i < nn->width[layer]; i++) {
			free(nn->weight[layer][i]);
			free(nn->weight_adj[layer][i]);
		}
		free(nn->weight[layer]);
		free(nn->weight_adj[layer]);
	}
	// There are no neurons in the input layer, as the input array itself is used to store these values.
	for (layer = 1; layer < nn->depth; layer++) {
		free(nn->neuron[layer]);
		free(nn->loss[layer]);
		free(nn->preact[layer]);
	}
	free(nn->weight);
	free(nn->weight_adj);
	free(nn->neuron);
	free(nn->loss);
	free(nn->preact);
	free(nn->bias);
	free(nn->activation);
	free(nn->width);
	free(nn);
}

// ---- ----------------- ----------- --------------------- -----------------------

int nn_add_layer(nn_t *nn, int width, int activation, int8_t bias)
{
    nn->depth++;
    nn->width = (int *)realloc(nn->width, nn->depth * sizeof(*nn->width));
    if (NULL == nn->width)
        return 1;
    nn->width[nn->depth - 1] = width;
    nn->activation = (int *)realloc(nn->activation, nn->depth * sizeof(*nn->activation));
    if (NULL == nn->activation)
        return 1;
    nn->activation[nn->depth - 1] = activation;
    nn->bias = (int8_t *)realloc(nn->bias, nn->depth * sizeof(*nn->bias));
    if (NULL == nn->bias)
        return 1;
    nn->bias[nn->depth - 1] = bias;
    nn->neuron = (int8_t **)realloc(nn->neuron, nn->depth * sizeof(int8_t *));
    if (NULL == nn->neuron)
        return 1;
    nn->loss = (int8_t **)realloc(nn->loss, nn->depth * sizeof(int8_t *));
    if (NULL == nn->loss)
        return 1;
    nn->preact = (int8_t **)realloc(nn->preact, nn->depth * sizeof(int8_t *));
    if (NULL == nn->preact)
        return 1;
    if (nn->depth > 1) {
        nn->neuron[nn->depth - 1] = (int8_t *)malloc(nn->width[nn->depth - 1] * sizeof(int8_t));
        if (NULL == nn->neuron[nn->depth - 1])
            return 1;
        nn->loss[nn->depth - 1] = (int8_t *)malloc(nn->width[nn->depth - 1] * sizeof(int8_t));
        if (NULL == nn->loss[nn->depth - 1])
            return 1;
        nn->preact[nn->depth - 1] = (int8_t *)malloc(nn->width[nn->depth - 1] * sizeof(int8_t));
        if (NULL == nn->preact[nn->depth - 1])
            return 1;
    }
    nn->weight = (int8_t ***)realloc(nn->weight, (nn->depth) * sizeof(int8_t **));
    if (NULL == nn->weight)
        return 1;
    nn->weight_adj = (int8_t ***)realloc(nn->weight_adj, (nn->depth) * sizeof(int8_t **));
    if (NULL == nn->weight_adj)
        return 1;
    if (nn->depth > 1) {
        nn->weight[nn->depth - 1] = (int8_t **)malloc((nn->width[nn->depth - 1]) * sizeof(int8_t *));
        if (NULL == nn->weight[nn->depth - 1])
            return 1;
        nn->weight_adj[nn->depth - 1] = (int8_t **)malloc((nn->width[nn->depth - 1]) * sizeof(int8_t *));
        if (NULL == nn->weight_adj[nn->depth - 1])
            return 1;
        for (int neuron = 0; neuron < nn->width[nn->depth - 1]; neuron++) {
            nn->weight[nn->depth - 1][neuron] = (int8_t *)malloc((nn->width[nn->depth - 2]) * sizeof(int8_t));
            if (NULL == nn->weight[nn->depth - 1][neuron])
                return 1;
            nn->weight_adj[nn->depth - 1][neuron] = (int8_t *)malloc((nn->width[nn->depth - 2]) * sizeof(int8_t));
            if (NULL == nn->weight_adj[nn->depth - 1][neuron])
                return 1;
            // Randomize the weights in this layer using uniform Xavier initialization
            // Range = +/- sqrt(6 / (#inputs + #outputs))
            for (int i = 0; i < nn->width[nn->depth - 2]; i++) {
                nn->weight[nn->depth - 1][neuron][i] = (int8_t)(sqrtf(6.0f / (nn->width[nn->depth - 1] + nn->width[nn->depth - 2])) * 128 * (rand() / (float)RAND_MAX - 0.5f));
            }
        }
    }
    return 0;
}

// Trains a nn with a given input and target output at a specified learning rate.
// The rate (or step size) controls how far in the search space to move against the
// gradient in each iteration of the algorithm.
// Returns the total error between the target and the output of the neural network.
int8_t nn_train(nn_t *nn, int8_t *inputs, int8_t *targets, int8_t rate)
{
    int16_t sum;
    int i, j, k;
    int16_t err = 0;

    nn->neuron[0] = inputs;
    forward_propagation(nn);
    // Perform back propagation using gradient descent, which is an optimization algorithm that follows the
    // negative gradient of the objective function to find the minimum of the function.
    // Start at the output layer, and work backward toward the input layer, adjusting weights along the way.
    // Calculate the error aka loss aka delta at the output.
    // Select last layer (output layer)
    i = nn->depth - 1;
    for (j = 0; j < nn->width[i]; j++) {
        // Calculate the loss between the target and the outputs of the last layer
        nn->loss[i][j] = error_derivative(targets[j], nn->neuron[i][j]);
        err += error(targets[j], nn->neuron[i][j]);
    }
    // Calculate losses throughout the inner layers, not including layer 0 which can have no loss
    for (i = nn->depth - 2; i > 0; i--) {
        for (j = 0; j < nn->width[i]; j++) {
            sum = 0;
            for (k = 0; k < nn->width[i + 1]; k++)
                sum += nn->loss[i + 1][k] * activation_function[nn->activation[i]](nn->preact[i + 1][k], true) * nn->weight[i + 1][k][j];
            nn->loss[i][j] = (int8_t)(sum / 128);
        }
    }
    // Calculate the weight adjustments
    // The weights cannot be updated while back-propagating, because back propagating each layer depends on the next layer's weights.
    // So we save the weight adjustments in a temporary array and apply them all at once later.
    for (i = nn->depth - 1; i > 0; i--)
        for (j = 0; j < nn->width[i]; j++)
            for (k = 0; k < nn->width[i - 1]; k++)
                nn->weight_adj[i][j][k] = (int8_t)((nn->loss[i][j] * activation_function[nn->activation[i]](nn->preact[i][j], true) * nn->neuron[i - 1][k]) / 128);
    // Apply the weight adjustments
    for (i = nn->depth - 1; i > 0; i--)
        for (j = 0; j < nn->width[i]; j++)
            for (k = 0; k < nn->width[i - 1]; k++)
                nn->weight[i][j][k] += (int8_t)((nn->weight_adj[i][j][k] * rate) / 128);
    return err;
}

// Returns an output prediction given an input
int8_t *nn_predict(nn_t *nn, int8_t *inputs)
{
    nn->neuron[0] = inputs;
    forward_propagation(nn);
    // Return a pointer to the output layer
    return nn->neuron[nn->depth - 1];
}

// Loads a neural net model file
nn_t *nn_load(char *path)
{
    FILE *file;
    nn_t *nn;
    int width = 0;
    int activation = ACTIVATION_FUNCTION_TYPE_NONE;
    int8_t bias = 0;
    int layer, i, j;
    int depth;

    file = fopen(path, "r");
    if (NULL == file)
        return NULL;
    nn = nn_init();
    fscanf(file, "%d\n", &depth);
    for (i = 0; i < depth; i++) {
        fscanf(file, "%d %d %hhd\n", &width, &activation, &bias);
        if (nn_add_layer(nn, width, activation, bias) != 0) {
            fclose(file);
            return NULL;
        }
    }
    // Read in the weights
    for (layer = 1; layer < nn->depth; layer++)
        for (i = 0; i < nn->width[layer]; i++)
            for (j = 0; j < nn->width[layer - 1]; j++)
                fscanf(file, "%hhd\n", &nn->weight[layer][i][j]);
    fclose(file);
    return nn;
}

// Saves a neural net model file
int nn_save(nn_t *nn, char *path)
{
    int layer, i, j;
    FILE *file;

    file = fopen(path, "w");
    if (NULL == file)
        return 1;
    fprintf(file, "%d\n", nn->depth);
    for (i = 0; i < nn->depth; i++)
        fprintf(file, "%d %d %hhd\n", nn->width[i], nn->activation[i], nn->bias[i]);
    for (layer = 1; layer < nn->depth; layer++)
        for (i = 0; i < nn->width[layer]; i++)
            for (j = 0; j < nn->width[layer - 1]; j++)
                fprintf(file, "%hhd\n", nn->weight[layer][i][j]);
    fclose(file);
    return 0;
}

uint32_t nn_version(void)
{
    return (NN_VERSION_MAJOR << 24) | (NN_VERSION_MINOR << 16) | (NN_VERSION_PATCH << 8) | NN_VERSION_BUILD;
}