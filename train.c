...
#define TARGET_TRAIN_ERROR 1 // 0.01 in fixed-point (0.01 * 128)
...
int main(void)
{
    ...
    int num_inputs = 256;
    int num_outputs = 10;
    int8_t learning_rate = 26; // 0.2 in fixed-point (0.2 * 128)
    int8_t annealing = 127; // 0.99 in fixed-point (0.99 * 128)
    int epochs = 0;
    // End of tunable parameters
    data_t *data;
    nn_t *nn;
    int j;
    int16_t train_error = 128; // 1.0 in fixed-point (1.0 * 128)
    ...
    while (train_error > TARGET_TRAIN_ERROR) {
        ...
        for (j = 0; j < data->num_rows; j++) {
            total_error += nn_train(nn, data->input[j], data->target[j], learning_rate);
        }
        epochs++;
        train_error = total_error / data->num_rows;
        printf("%.5f, %.5f\n", train_error / 128.0, learning_rate / 128.0); // Convert fixed-point to float for display
        learning_rate = (int8_t)((learning_rate * annealing) / 128); // Adjust learning rate using fixed-point arithmetic
        // Incremental save
    ...
    data_free(data);
    nn_free(nn);
    printf("Final train error: %.5f\n", train_error / 128.0); // Convert fixed-point to float for display
