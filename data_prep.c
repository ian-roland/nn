...
data_t *data_init(int num_rows, int num_inputs, int num_outputs)
{
    ...
    data->input = (int8_t **)malloc(data->num_rows * sizeof(int8_t *));
    if (NULL == data->input) {
        free(data);
        return NULL;
    }
    data->target = (int8_t **)malloc(data->num_rows * sizeof(int8_t *));
    if (NULL == data->target) {
        free(data->input);
        free(data);
        return NULL;
    ...
    for (int i = 0; i < data->num_rows; i++) {
        data->input[i] = (int8_t *)malloc(data->num_inputs * sizeof(int8_t));
        ...
        data->target[i] = (int8_t *)malloc(data->num_outputs * sizeof(int8_t));
...
void data_parse(data_t *data, char *line, int row)
{
    for (int column = 0; column < (data->num_inputs + data->num_outputs); column++) {
        ...
        int8_t fixed_val = (int8_t)(val * 128); // Convert float to fixed-point
        if (column < data->num_inputs)
            data->input[row][column] = fixed_val;
        else
            data->target[row][column - data->num_inputs] = fixed_val;
    }
}
