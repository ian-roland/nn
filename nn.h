...
typedef struct nn {
    ...
    int8_t *bias;         // Biases by layer (each layer can have its own bias)
    int8_t **neuron;      // Output value for each neuron in each layer
    int8_t **loss;        // Error derivative for each neuron in each layer
    int8_t **preact;      // Neuron values before activation function is applied for each neuron in each layer
    int8_t ***weight;     // Weight of each neuron in each layer
    int8_t ***weight_adj; // Adjustment of each weight for each neuron in each layer
} nn_t;
